...
function compare_object(obj1,obj2)
{
   ...
   Object.keys(obj1).forEach((value) => {
     if(obj1[value] !==obj2[value] && !(value in Keys_obj2) ){
        falge=false;
        return falge;
     }
   });
...
const csvTo2DArray = (csvString) => csvString.split("\n").map((row) => row.split(","));
...
const objectArrayToCSV = (data, columns) => {
  const headers = columns.join(",");
  const rows = data.map((obj) => columns.map((col) => obj[col]).join(","));
  return `${headers}\n${rows.join("\n")}`;
};

// 6. Write a JavaScript program to target a given value in a nested JSON object based on the given key.
const getNestedValue = (obj, key) => key.split(".").reduce((value, k) => (value && typeof value === "object") ? value[k] : undefined, obj);

// 7. Write a JavaScript program to convert a specified number into an array of digits.
const numberToArray = (num) => Array.from(String(num), Number);
...
const filterOut = (arr, ...values) => arr.filter((item) => !values.includes(item));
...
const minMaxValue = (arr, fn) => arr.reduce(([min, max], value) => {
  const result = fn(value);
  return [Math.min(min, result), Math.max(max, result)];
}, [Infinity, -Infinity]);
